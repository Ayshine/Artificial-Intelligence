...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            #getting the list of literals in the looked level
            literals = self.s_levels[level] 
            action_node = PgNode_a(action)
            precond_for_node = action_node.prenodes
            #check if all precond for actions are in literals
            #check with simple for loop or with function "issubset" and convert both lists to sets
    ...
    def add_literal_level(self, level):
        ...
        for action in self.a_levels[level - 1]:
            ...
            #getting nodes effected by current action 
        ...
        #some actions are have same states, so we are having duplicates in literal_level
        #adding set(literal_level)  to remove all duplicates
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        #getting parents of nodes
        parents_a1 = node_s1.parents
        parents_a2 = node_s2.parents
       
        counter_of_mutex = 0

        #For each parent from a first node check all nodes in parent states from second node
        for parent_a1 in parents_a1:
            for parent_a2 in parents_a2:
                if parent_a1.is_mutex(parent_a2):
                    #if those two parents are mutax  add one to our counter
                    counter_of_mutex += 1
    ...
    def h_levelsum(self) -> int:
       ...
       #Defining starting variables
        ...
        for layer_index in range(num_of_layers):
            ...
            #break from loop if we find goal using goal_check
            goal_check = False
            #going through all nodes in current level
            for node in current_level_states:

                #checking for each goal in the problem 
